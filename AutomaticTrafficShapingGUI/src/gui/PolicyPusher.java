...
public class PolicyPusher {
	...
	public static void pushPolicy(String policyName, String payload){
		...
		try {
			oracle = new URL(ConfigReader.getControllerURL()+"/restconf/config/opendaylight-inventory:nodes/node/vRouter-R1/yang-ext:mount/vyatta-policy:policy/vyatta-policy-qos:qos/"+policyName);
			...
			connection.setRequestProperty("Accept","*/*");OutputStreamWriter osw =new OutputStreamWriter(connection.getOutputStream());
			osw.write(payload);
			osw.flush();
			osw.close();
